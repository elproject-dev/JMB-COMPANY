...
export default function PenjualanPage() {
  ...
  const confirmEdit = async () => {
    ...
    if (oldItem && oldItem.dompet_id) {
      if (selisih !== 0) {
        ...
        if (freshDompet) {
          // Penjualan: selisih > 0 berarti nominal masuk bertambah -> saldo nambah
    ...
    if (selisih !== 0) {
      const jenisSelisih = selisih > 0 ? "penyesuaian_masuk" : "penyesuaian_keluar"
      await supabase.from('kas').insert({
        tanggal: format(new Date(), 'yyyy-MM-dd'),
        keterangan: `Revisi Penjualan - ${editPelanggan}`,
  ...
  const confirmHapus = async () => {
    ...
    if (deletingTransaksi.dompet_id) {
      ...
      if (freshDompet) {
        // Hapus penjualan -> uang yang tadinya masuk, harus dikeluarkan lagi -> kurang saldo
    ...
    await supabase.from('kas').insert({
      tanggal: format(new Date(), 'yyyy-MM-dd'),
      keterangan: `Penghapusan Transaksi Penjualan - ${deletingTransaksi.pelanggan}`,
      jumlah: delJumlah,
      jenis: "penyesuaian_keluar"
    })
    ...
    toast.add({
      ...
      description: `Transaksi dengan pelanggan "${deletingTransaksi.pelanggan}" telah dihapus dan saldo dompet telah dikurangi.`,
    })
      ...
      <AlertDialog open={!!deletingTransaksi} onOpenChange={(open) => !open && setDeletingTransaksi(null)}>
        <AlertDialogContent className="rounded-none">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin menghapus transaksi penjualan dari <strong>"{deletingTransaksi?.pelanggan}"</strong>? Tindakan ini tidak dapat dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
