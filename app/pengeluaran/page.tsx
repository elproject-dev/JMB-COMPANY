...
export default function PengeluaranPage() {
  ...
  async function loadData() {
    ...
    const { data: katData } = await supabase.from('kategori_kas').select('nama').eq('jenis', 'pengeluaran')
  ...
  const handleSimpanKeteranganBaru = async () => {
    ...
    if (!keteranganOptions.includes(searchKeterangan)) {
      await supabase.from('kategori_kas').insert({ nama: searchKeterangan, jenis: 'pengeluaran' })
  ...
  const handleSimpan = async () => {
    ...
    if (!editingId && dompetObj) {
      ...
      if (freshDompet && Number(freshDompet.saldo) < parsedJumlah) {
        toast.add({
          title: "Gagal",
          description: `Saldo dompet/rekening kurang untuk mencatat pengeluaran ini.`,
        })
        return
      }
    }
    ...
    toast.add({
      title: "Berhasil",
      description: editingId ? "Data pengeluaran telah diperbarui." : "Data pengeluaran telah disimpan.",
    })
  }
  ...
  const handleDeleteAll = async () => {
    ...
    // Soft delete all pengeluaran
    await supabase.from('kas').update({ is_deleted: true }).eq('jenis', 'pengeluaran')

    await loadData()
    setIsDeletingAll(false)
    toast.add({
      title: "Berhasil",
      description: "Semua riwayat pengeluaran telah dihapus.",
    })
  }
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-1 flex-col p-4 md:p-6 w-full">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-4">
              <h1 className="text-1xl font-bold tracking-tight">Daftar Pengeluaran</h1>
            </div>
            <div className="flex gap-2 items-center">
              ...
              >
                {isAdding ? "Batal" : "Tambah Pengeluaran"}
              </Button>
          ...
          {isAdding && (
            <div className="mb-6 p-6 bg-card border rounded-none shadow-sm">
              <h2 className="text-sm font-semibold mb-4">{editingId ? "Edit Pengeluaran" : "Tambah Pengeluaran Baru"}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  <Label>Sumber Dana (Diambil Dari)</Label>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="border-r whitespace-nowrap">Keterangan / Keperluan</TableHead>
                  ...
                  {kas.length === 0 ? (
                    ...
                    currentKas.map((k, index) => (
                      <TableRow key={k.id}>
                        ...
                        <TableCell className="border-r text-right font-semibold text-red-600">-{formatRibuan(String(k.jumlah))}</TableCell>
      ...
      <AlertDialog open={!!deletingKas} onOpenChange={(open) => !open && setDeletingKas(null)}>
        <AlertDialogContent className="rounded-none">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin menghapus data pengeluaran sebesar <strong>Rp {formatRibuan(String(deletingKas?.jumlah || ""))}</strong>? Tindakan ini tidak dapat dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={isDeletingAll} onOpenChange={setIsDeletingAll}>
        <AlertDialogContent className="rounded-none">
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Semua Pengeluaran?</AlertDialogTitle>
            <AlertDialogDescription>
              Apakah Anda yakin ingin menghapus <strong>seluruh data pengeluaran</strong>? Saldo dompet akan disesuaikan kembali. Tindakan ini tidak dapat dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="rounded-none">Batal</AlertDialogCancel>
